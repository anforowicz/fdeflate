...
fuzz_target!(|input: &[u8]| {
    let r_whole = decompress_by_chunks(Decompressor::new(), input, std::iter::repeat(input.len()), false);
    let r_bytewise = decompress_by_chunks(Decompressor::new(), input, std::iter::repeat(1), false);
